...
| SCOPE | 含めるブロック | N の解釈 |
| --- | --- | --- |
| `dialogue` (default) | user/assistant のプレーンテキストのみ | filter 後の dialogue ブロック末尾 N 件 |
| `full` | dialogue に加えて thinking / tool_use / tool_result も含める。tool 呼び出しはサマリ付き fold にまとまる | user/assistant JSONL レコード末尾 N 件 |

dialogue モードでは先に内部ブロックを除外してから末尾 N を取るため、tool 呼び出しに埋もれて dialogue が 1〜2 件しか残らないという問題が起きない。
...
transcript 処理と vim 起動を 2 スクリプトに分離してある。
...
3. MODE が未指定なら tmux 環境を判定して自動決定 (tmux 内 → `window`、tmux 外 → `print`)。問い合わせなし
