...
session-closing plugin は 3 skill を持つ。役割が違うので混同しない。
...
| session-close | retrospective → handover を順に実行 | 両 skill の出力の合算 |

session-close 経由でも、retrospective / handover を単独で呼ぶ運用も OK。
...
- 懸念・リスク・メモ
- やり残し・拾い残し: 完了予定だったが終わらなかったもの、
  会話中に言及されたがタスク化も実行もされなかったものを収集する
...
- `8. やり残し・拾い残し`: やり残し (完了予定だったが未完了) と
  拾い残し (言及されたが未着手) を記載する。なければセクションごと省略
- `9. 再開手順`: 実際の step 数に応じて `### Step N` を増減させる
