...
合成版を書く際、以下のアンチパターンに該当する記述を含めない:

- 他の rule/skill/agent への補足的な相互参照 (例: 「詳細は xxx を参照」「meta-review が検証する」)。参照がなくても成立する自己完結した rule にする。例外は定義の完全な委譲のみ (例: skill-authoring と ref-skill-authoring の境界線)
- 外部ツールの具体的な設定名・ルール名 (例: markdownlint の `no-bare-md-references`)。rule は禁止事項を宣言するだけでよく、検出手段は結合しない
- プロジェクト固有の運用慣習を汎用 rule として書く (例: 特定 repo の「README 機能セクションが source of truth」を全 repo に適用)

#### review マーカー

合成版の中で、元ファイルから変更した箇所の近くに review マーカーを挿入する。マーカーはコメント (変更理由) を囲むだけで、変更後の内容自体はマーカーの外に普通に書く:
...
ユーザーは staging ファイルをエディタで確認し、OK ならマーカーごと削除する。質問があれば `@claude` でマーカー内にコメントを書いて返す。
...
クラスタ内のファイルを分析した結果、異なる関心事が混在していると判定した場合に提案する。例: 同じ rule の中に汎用 markdown 規約と design-doc 固有の引用規約が同居している場合。

```markdown
### {元ファイル名} → {分割先 A} + {分割先 B}
理由: {なぜ分割が望ましいか (paths 対象が異なる、読者/用途が異なる等)}
影響 repo: [{repo}, ...]
分割先 A の概要: {内容}
分割先 B の概要: {内容}
```

分割提案がある場合、合成版ドラフトは分割前のベストバージョンとして出す。分割の実施はユーザー判断。
