...
user-invocable: true
allowed-tools: Monitor, Bash, TaskList, TaskStop
...
最終活動から 3300 秒 (55 分) 以上経過した時だけ stdout に 1 行出力する。
...
1. TaskList で description に `cache-keepalive` を含むタスクを探す
2. 既にある → 「cache-keepalive は既に有効です」で終了
...
1. TaskList で description に `cache-keepalive` を含むタスクを探す
2. ある → TaskStop(id) し「cache-keepalive を停止しました」と報告
3. ない → 「現在有効な cache-keepalive はありません」と報告

### status / state / list

1. TaskList で description に `cache-keepalive` を含むタスクを探す
2. ある → 「cache-keepalive: 有効」と報告
...
- 3300 秒以上経過 → 1 行出力 (Claude への keepalive 通知)、その後 3300 秒 sleep
- 3300 秒未満 → `(3300 - elapsed + 10)` 秒 sleep して再チェック
    - +10 はバッファ。sleep 後の再チェックで確実に 3300 を超えるようにする
