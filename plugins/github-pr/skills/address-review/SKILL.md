...
1. コメントの内容を解釈し、対象ファイルと修正方針を決める。複数行コメント（`start_line` が非 null）は
   `start_line`〜`line` の範囲全体が対象で、末尾の行だけを指摘と読まない。diff が更新されて両方が null に
   なったコメントは `original_start_line`〜`original_line` で範囲を取る
...
- 実施したことは commit hash か実施の記録を添えて書く。まだ実施していないことを予定として書くのはよいが、
  いつ・どこで（どの PR、どのタイミング、main 直接か）実施するかを具体的に書く。
  「〜します」だけでは、計画に載せただけか実施したかがレビュアーに分からない。
  ユーザーの承認が要る変更（設計判断・rule / skill の変更・公開される操作）は先に実行せず、予定として書いて確認を待つ
...
- {commit_sha}: {指摘内容の 1 文要約}（[コメント]({コメント URL})）
- {commit_sha}: {指摘内容の 1 文要約}（[コメント]({コメント URL})）。横展開: {commit_sha}
- {commit_sha}: 次の 2 件を同じ commit で対応（同じ行への指摘で差分が衝突するため）
    - {指摘内容の 1 文要約}（[コメント]({コメント URL})）
    - {指摘内容の 1 文要約}（[コメント]({コメント URL})）
