...
3. ファイルを修正（グループ内の全コメントと、横展開で見つけた同型を一括で反映）
4. `git add` → `git commit`（グループの修正内容を要約したメッセージ）
5. `git push`
6. グループ内の各コメントに返信（ステップ 6 の形式）。横展開で直した分は、
   返信にその旨と件数を書く
7. 対応した各コメントに rocket リアクションを付ける（対応済みマーク）
