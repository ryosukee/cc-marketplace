...
export function loadSource(jsonPath) {
  ...
  if (src.formIntro != null && (src.type !== "form" || !Array.isArray(src.formIntro) || !src.formIntro.length)) add("formIntro", "formIntro は form だけが持つ空でないブロックの配列（このフォームについて）");
...
export function renderPage(src, opts = {}) {
  ...
  // 脚注・補足の採番。本文の並び順（節 → 参考資料 → 生成に関する補足 → 前提・form の説明・report のまとめ → 脚注と補足の本文）
  const fnKeys = Object.keys(src.footnotes || {}), suKeys = Object.keys(src.supplements || {});
  const order = [];
  const seen = new Set();
  ...
  if (isForm) collectBlocks(src.formIntro || []);
  ...
  if (isForm && Array.isArray(src.formIntro) && src.formIntro.length) {
    ...
    parts.push(`<div class="summary">\n<span class="eyebrow">このフォームについて</span>\n${introHtml}\n</div>`);
  } else if (!isForm) {
    at("summary");
