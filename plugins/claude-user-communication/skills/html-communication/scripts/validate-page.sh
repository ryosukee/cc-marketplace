...
# 4 層の検査を順に実行し、指摘を JSON で stdout に集約する。
#   1. html-validate@11 (構文・a11y・document。ルールは同梱の htmlvalidate.json)
#   2. linkinator@8 --check-fragments (リンク・同一ページ fragment の存在)
#   3. check-page.mjs (雛形固有の検査 19 種: 図の CSS custom property・フォント段階・セル長・caption 対応・脚注対応・
...
#   4. check-source.mjs (生成元 JSON の検査 7 種。src/{stem}.json があるページだけ:
#      書式・参照の解決・タグの混入・番号の直書き・図の対応・パターン集の CSS・
#      HTML との食い違い)
# 引数は閲覧用 HTML のパス。生成元は同じディレクトリの src/ から探す
...
SOURCE_CHECKER="$SCRIPT_DIR/check-source.mjs"
...
# 4. 生成元 JSON の検査（src/{stem}.json があるページだけ）
: > "$tmp/source.jsonl"
for f in "$@"; do
  s="$(dirname "$f")/src/$(basename "$f" .html).json"
  [ -f "$s" ] || continue
  rc=0; node "$SOURCE_CHECKER" "$s" > "$tmp/src.json" || rc=$?
  if [ "$rc" -ge 2 ]; then
    echo "check-source.mjs が失敗 (exit $rc): $s" >&2
    exit 2
  fi
  jq -c '.results[]' "$tmp/src.json" >> "$tmp/source.jsonl"
done
...
jq -n \
  ...
  --slurpfile source <(jq -s . "$tmp/source.jsonl") '
  {
    ...
    total: (
      ...
      + ([$source[0][].findings | length] | add // 0)
    )
  }' > "$tmp/out.json"
