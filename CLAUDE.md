...
| plane-kanban | kanban | Plane（kanban）の work item を repo 単位の project で読み書きする。skill 1 本と、REST API を直接呼ぶ bash スクリプト（一覧・作成・state の更新・一括取り込み）。セッションは label で残す。macOS 限定で、API key は Keychain、workspace slug は環境変数から読む（plugin README を参照） |
