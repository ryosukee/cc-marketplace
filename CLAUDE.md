...
| session | 2.11.1 | session | start/debrief/retrospective/handover/end + handover の機械検査 9 種 + handover-reviewer agent |
...
| claude-user-communication | 0.24.0 | communication | HTML ページ提示 (claude-html-communication) + 選択肢形式の確認 + 生成ページの機械検査 12 種 + 提示前レビューの page-reviewer agent。雛形は 1 / 2 / 3 pane + 現在地の追従。セクション番号は見出しの外に出す。図は Tailwind 可 (図の中だけ)。要環境変数 (plugin README) |
