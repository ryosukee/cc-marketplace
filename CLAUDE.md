...
| claude-user-communication | 0.32.5 | communication | HTML ページ提示 (claude-html-communication) + 選択肢形式の確認 + 生成ページの機械検査 15 種 + 提示前レビューの page-reviewer agent。雛形は 1 / 2 / 3 pane + 現在地の追従 + 設問のグループ化。セクション番号は見出しの外に出す。図は Tailwind 可 (図の中だけ)。要環境変数 (plugin README) |
