...
| diffo | 0.4.0 | authoring | 参照知識 skill `ref-diffo` 1 本。レビューを Claude Code の background task または Codex の `queue` で作業セッションへ届ける。返信先の取り方、指定文言の反映、ターミナルへの重複報告の抑止、markdown プレビューと解決済みスレッドの表示切替も扱う |
