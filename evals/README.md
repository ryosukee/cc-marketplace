...
- 消費はサブスクのレート枠（OAuth 経由）。実測 1 実行あたり $0.3〜0.9
  （2026-09-21、github-pr の 27 ケース・69 実行で $27.50）。
  実行は直列で、`-j` で最大 8 並列にできる
...
- 引数なしの `mktemp` は macOS では `TMPDIR` ではなく `/var/folders` の下を使う。
  ...
  （2026-09-18 に実測）
- stub が呼ぶ外部コマンドは、子セッションで効くとは限らない。`diff` と `grep` を使った判定が
  常に失敗側へ倒れる例があった（2026-09-21 に github-pr で実測）。
  判定は bash の文字列比較で済ませ、外部コマンドは `git` と `jq` にとどめる
...
- 動作の評価の `max_turns` は、依頼を最後まで終えられる数にする。
  ...
  ターン上限の error が出てもスコアは付く（2026-09-21 に github-pr で実測）
...
- 1 行 1 呼び出しで `key=value` を空白で区切って並べる。生の引数は別のファイルへ分け、
  落ちたケースを人が読むためだけに使う
- 順序や前後関係を測るなら、その時点の状態を stub が真偽で書く
  （push 済みか、作業ツリーが開始時点から変わっているか）。grader 側で組み立てない
- 本文を別のファイルへ追記するときは、区切りの行に記録と同じフィールドを載せる。
  本文の判定がそのファイルだけで完結する
- 正規表現でフィールドの値を指すときは、値の直後に区切りの空白を要求する。
  要求しないと `target=c2` が `target=c20` に当たる
...
- 同じ plugin に skill が 2 つ以上あるときは、一方の正例を他方の負例にする。
  ...
  （指摘を直して push した後に PR 本文の更新へ入るなど）
...
実測では一覧に 16 ラウンド、実装に 15 ラウンドかかった（2026-09-21、github-pr）。
ラウンドを重ねるほど指摘は細かくなるが、`target_dirty` が常に `yes` になる stub の不具合は
実装のレビューではなく実行で見つかった。レビューは回すが、回しただけで通ったことにしない。
