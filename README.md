...
mj-tools version-check summaries [LIMIT] 保存済み changelog 要約の一覧
mj-tools version-check summary <VERSION> 指定バージョンの changelog 要約を表示
...
`internal/` は cache 内のバージョン付きディレクトリに存在する（例: `cache/mj-tools/version-check/0.4.0/internal/`）。plugin バージョンが上がると新しい cache ディレクトリが作られるため、旧バージョンの状態データは取り残される。
