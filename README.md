...
- impl-spec (0.5.3) — 実装のための仕様策定 plugin。3 skill + 1 agent を提供
...
- dotclaude-writer (0.3.1) — `.claude/` protected directory への書き込みワークアラウンド。staging 経由のスクリプトで create/edit/rm を提供。main session と `claude -p` で動作（subagent は skill 不可視のため非対応）
