...
norm-refit の中で次に着手するのは段階 R で、起点は op-review を単機能 plugin として新設すること。**
...
着手の時期は未定で、順序は暫定。
同日、その調査から派生してエイリアスの条項の対処を ccm-f063 で 5 件確定した。
反映先は `rules/` 2 ファイルと plugin 2 つで、実装は 1 本の PR にする（着手前）。
入れる 4 機構は、効いたかどうかを測る手段を持たないまま入るので段階 V の測定対象になる
（「段階 V」の節）。
