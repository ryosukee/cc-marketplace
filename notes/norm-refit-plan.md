...
2026-09-01 時点。**段階 1・2・3-1 が完了し、段階 V を新設した。段階 3 は 3-1（実装の PR 群）と
...
（「段階 V」の節）。
R1 は、2 実装の突合で計画の前提が 2 つ崩れたため、着手前に決める 5 件を ccm-f064 で確認中
（「段階 R」の R1 の節）。
...
着手前に決める 5 件を ccm-f064 で確認中。plugin 名 / 判定の粒度 / 改善提案の抽出 /
facet カタログの同梱範囲 / markdownlint の実行。
