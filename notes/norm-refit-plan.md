...
ただしセッションの着手順はこれで決まらない。norm-refit の外の作業が先に来ることがある。
段階 V は、条項が載っていても引かれない事象の調査から新設した（2026-09-01）。
norm-refit の到達点 1〜5 が全部成立しても残る問題なので、独立した段階として立てた。
着手の時期は未定で、順序は暫定。
同日、その調査から派生してエイリアスの条項の対処を ccm-f063 で 5 件確定し、
実装を PR #18 として main `7f63be7` にマージした（session 2.13.0 / claude-user-communication 0.39.0）。
入れた 4 機構は、効いたかどうかを測る手段を持たないまま入っているので段階 V の測定対象になる
（「段階 V」の節）。
R1 は、着手前に決める 5 件が ccm-f064 で確定した（2026-09-02）。plugin 名は `norm-review`、
判定の粒度は付けず 2 値のまま、改善提案の抽出を取り込み、facet カタログはスキーマ +
テンプレート 2 件、markdownlint の実行は落とす。確定の実文は `notes/norm-refit.md` の
2026-09-02 のエントリ 3 件にある。実装は未着手（「段階 R」の R1 の節）。
2026-09-07 に、norm-refit の外で claude-user-communication へ `sentence-reviewer` を入れた
（PR #19 / #20、0.42.0）。文脈を持たない agent に「誰が何をどうするか」の取り出しを課す形で、
...
| V | 規範が読まれ、守られるかを測る仕組み | 条項が読み込まれるところと、読み込まれた後で守られるところの両方が測れる |
| 4 | T4 の設計と実装 | 到達点 4 が成立し、保留 18 件が解ける |
| 5 | 完了処理 | 編集凍結が解け、norm-refit が終わる |

段階 3 を 3-1 と 3-2 に割り、あいだに段階 R を入れた（ユーザー判断 2026-09-01）。
3-2（保守の手引き）は最終構成が固まらないと書けず、段階 R が規範の読ませ方を変えるため、
R の後でないと着手できない。3-1 は残る 2 本が両方とも外へ出たので閉じた。

段階 3-2 より後は暫定で、段階 R が終わった時点で組み直す。
...
2026-08-31 に PR 5 を段階 R の R7 へ移し、2026-09-01 に PR 7 を段階 3-2 へ移した。
残る 2 本が両方とも外へ出たので、段階 3-1 は 7 本のマージをもって閉じた。
...
#### PR 5 review agent の責務統一（段階 R へ移した）

2026-08-31 に段階 R の R7 へ移した（ccm-f056 Q1）。終了条件を揃える部分は R3 が先に決める。
中身は「段階 R レビュー機構の最適化」の R7 にある。
...
PR 5・段階 4 の「レビュー機構のゼロベース再設計」・v2 群 A3 を解体し、1 つの段階として設計し直したもの
（ccm-f056、2026-08-31）。旧 3 分類の切り分けは使わない。

着手の起点は R1。R2 が決まるまで R3・R4・R6 は判定できないので、順序を守る。
...
- 文レベルの欠陥を agent がほとんど捕まえていない（事例集 39 件のうち agent 由来は 1 件）。
  2026-09-07 に、文脈を持たない `sentence-reviewer` を html-communication へ先行して入れた（R4 で扱う）
...
- KAO 側（`kanban-agent-orchestrator` の SKILL.md 169 行 + facet 定義 166 行）から、
  efso に無いもの（改善提案の抽出、深刻度の 3 段階、「普遍的な原則も norm」の 1 文）を取り込む
- 中身は agent + 作業手順 skill + facet の定義。既存 7 本の agent と同じく plugin に属させる
- 依存なし

2 実装の突合で、この節の前提が 2 つ崩れた（2026-09-01）。

- **facet の中身 14 件はすべて対象プロジェクト固有で、そのままは持ち出せない。**
  持ち出せるのはカタログのスキーマまで。ほぼ持ち出せるのは
  efso の `security-practice` と KAO の「PR レビューフィードバック」の 2 件だけになる
- **KAO 側に agent は無い。** `.claude/agents/` は空で、KAO は workflow の step へ
  skill を注入して使う。agent 層は efso にしかない

上の「efso に無いもの 3 つ」の列挙は、ccm-f056 の回答実文（「efso を基準に KAO の差分を
取り込んで 1 本にする」の 1 行）には無く、この計画が判断材料から具体化したものになる。
このうち深刻度の 3 段階は、efso の agent 定義の「重要度のランク付けはしない」と衝突する。

着手前に決める 5 件は ccm-f064 で確定した（2026-09-02）。

- plugin の名前は `norm-review`
- 判定の粒度は付けない（2 値のまま）
- 改善提案の抽出を取り込む
- facet カタログはスキーマ + テンプレート 2 件を同梱する
- markdownlint の実行は落とす

あわせて 2 件が決まった。カタログは汎用と example の 2 層に分け、一般的にどこでも使えるものを
本体として取り込み、特定のドメインでしか使えないものは example と分かる形で参考資料に置く。
norm カタログも同じ形で持つ。

未確定が 1 件残る。他の rule / skill を名指しせず、実行時に見えるものから norm になるものを
探す工程を置くかどうか。確定の実文と、この 3 件の内訳は `notes/norm-refit.md` の
2026-09-02 のエントリ 3 件にある。

### R2 既定の facet と norm set を設計する

- cc-marketplace で何を facet にするか。KAO の 5 facet はコードと workflow 実行データ、
  efso の 9 facet は認証設計の文書が対象で、どちらもそのままは当たらない
- どの rule を norm として読み込ませるか。レビュー観点だけでなく、
  対象ファイルを触るときに発火する rule も norm に含めるかを決める
- 依存: R1
...
- ユーザー判断 2026-09-01「危険ワードを列挙するのは op-review の最適化において、
  ...
  100% クソゴミ抽象エイリアスワードとは限らないし。
  op-review 最適化のタスクの中で適切に議論設計して入れ込むべき」

### R3 終了条件を決める

- facet や norm によって変えるかを含めて決める。norm によっては指摘 0 件まで繰り返すのが
  現実的なものもある（ccm-f056 Q4）
- 決めたら、旧方式で残っている spec-reviewer と impl-spec 3 skill（「指摘がゼロになるか、最大 5 回」）と、
  記述の無い PR 前のセルフレビューを揃える
- 依存: R2。既定の facet と norm set が決まらないと、どの単位で条件を変えるかを判定できない

### R4 既存 reviewer agent 5 本の扱いを比較検討する
...
- index の一覧は直近 10 件までを出し、それより前は別に作るアーカイブページへ遷移させる
...
旧 PR 5 の中身。段階 2 Q5 と ccm-f051 Q5 の確定を実装する。

- `handover-reviewer.md` と `page-reviewer.md` から「どう直すか」を落とす。
...
1. 段階 3-2（japanese-text-writing の保守の手引き）。段階 R で規範の読ませ方が変わるので、
   その後に中身を組み直す
2. 段階 4 の残り（情報デザイン norm 48 本の置き場、参考デザイン例の reference、独立の設計入力 2 件）。
   レビュー機構の再設計が段階 R へ抜けたので、残りは互いに独立で順序は任意
3. 段階 5 完了処理（facet 参照の切り替え → 編集凍結の解除 → 到達点の検証）。
   実装の完成を待つのはここだけ
...
段階 3-1 の PR 7 として設計していたものを、2026-09-01 に独立した段階へ移した。
着手は段階 R の完了後で、そのときに中身を組み直す。以下は移す前の骨子で、そのまま採るとは限らない。

再編後の japanese-text-writing は修正・追加が頻繁に入る想定（retrospective の codify、
外部で話題の規範の取り込みなど）。その際に今回の再編で守った設計を壊さないための
手引き rule を作る。最終構成が固まってからでないと書けない。
...
- 入口 rule の誘導を分類別まで届かせる: 現在の分岐は `core.md` までで止まり、
  分類別へ辿る指示は `core.md` の中にしかない。2026-09-05 に 2 段目が実行されない事例が出た
  （段階 V の「載る前に落ちる経路が 1 件見つかった」）。
  入口に分類判定まで持たせるのか、`core.md` の 3 ステップ目を強めるのかを決める
- 外部から入口 rule を参照している箇所の名前を揃える:
  `html-communication` の SKILL.md 347 行が「日本語テキストの執筆要点」と呼んでいるが、
  実際の H1 は「日本語テキストの執筆規範」
...
## 段階 V 規範が読まれ、守られるかを測る仕組み

条項がコンテキストに載るところと、載った状態でその条項が振る舞いを変えるところの
両方を測る仕組みを作る。測定そのものは今回は流さない（ユーザー判断 2026-09-01）。
この段階で決めるのは測り方で、測るのは仕組みができてからになる。

立てた当初は後半（載った状態で守られるか）だけを対象にしていた。
2026-09-05 に、条項が載る前に落ちる経路が見つかったので前半を足した（下記）。

番号ではなく `V`（verification）を振るのは、段階 R と同じ理由による。
既存の段階 1〜5 の番号を他の文書が参照しており、振り直すと参照がずれる。
v2 の群 A〜D とも重ならない文字を選んだ。
...
- 判定のきっかけも常に載っていた。入口の `rules/japanese-text-writing.md` は
  `paths` を持たない常時ロードで、「判定は書き出す直前に行う」を含む

差はテキストの有無ではなく、引きにいったかどうかにある。
これは norm-refit の到達点 1〜5 が全部成立しても残る。
実体を 1 箇所へ集め、矛盾を消し、層を実装しても、引かれなければ振る舞いは変わらない。

あわせて 2 つの数字が出ている。常時ロードは 12 本・37,360 バイト・条項 155 件で、
user global の常時分は 1 か月で約 1.8 倍になった。
同時に満たせない条項の組が 7 件あり、2026-09-01 のセッションで 2 件を実際に踏んでいる。

### 載る前に落ちる経路が 1 件見つかった（2026-09-05）

この段階を立てた根拠は「条項は 3 回とも載っていた。載っていなかった時点は 0 件」だった。
2026-09-05 に、その前提が成立しない事象が出た。

HTML フォーム ccm-f076 を書くとき、入口の `rules/japanese-text-writing.md` の分岐に従って
`core.md` を読んだが、`core.md` の「使い方」が指示する 3 ステップ目
（該当する分類の詳細規範を読み、個別規範を重ねる）を実行せず、
`decision-docs.md` を読まないまま提示した。ユーザーから「全般的に日本語がおかしい」
「なんで分類別読まない？」として差し戻された。

条項が守られなかったのではなく、条項が載っていなかった。
段階 V が測ろうとしている「載った状態での遵守」とは別の事象になる。

読まれずに素通りした経路の候補は 8 件を `notes/norm-refit.md` の未解決課題へ挙げた。
うち 3 件はファイルを見て特定できている。

- 入口 rule の分岐は `core.md` までしか案内しておらず、分類別へ辿る指示は `core.md` の中にしかない
- `html-communication` の SKILL.md は情報設計として product-boilerplate の `docs/norms/` を指し、
  japanese-text-writing の分類別へ誘導していない。skill の手順どおりに作ると読む契機が無い
- その SKILL.md 347 行は入口 rule を「日本語テキストの執筆要点」と呼ぶが、
  実際の H1 は「日本語テキストの執筆規範」。名前で辿れない

この段階では測り方だけを決める。直す作業は段階 3-2（入口の誘導）と段階 R（検知）に分かれる。
...
対照条件が無いので「条項が振る舞いを変えていない」ことの証明にはならない。
言えるのは「載せただけでは止まらない」ことだけで、
この段階が作る対照つきの測定はこの観測では代替できない。

一方で、この観測は課題文の作り方に使える材料になる。
条項の違反が語形で数えられる（汎用名詞 + 数字、数字と英字を繋いだ記号）ので、
grader が機械で判定できる条項の実例になる。
...
| 機構 | 置き場 | 効いたと言える条件 |
| --- | --- | --- |
| 入口 rule への 1 行 | `rules/japanese-text-writing.md` | 会話の返答での出現が減る |
| 条項の例の追加 | `core.md` の「読み手の負荷」 | 同上（入口 rule と分離して測れるかは未決） |
| レビュー手順の段 | `page-reviewer` の手順 | 提示前に指摘が出る |
| 引き継ぎの機械検査 | `check-handover.mjs` | 持ち越しの時点で落ちる |

前 2 つは「載せる」系統で、この段階が疑っている系統そのものになる。
後 2 つは検出の系統で、指摘・検出の件数がそのまま測定値になる。
...
同じ手が使えるのはこの 1 件だけになる。`# Delivering work`（案を提示してから実装する、と競合）と
`# Corrections`（未完了を紛れさせない、と競合）の環境変数は強制 ON しかできず、
Opus 5 では GrowthBook の既定に関わらず入る。3 つを分離して測ることはできない。
...
- 課題文をどう選ぶか。条項ごとに別の課題文が要るのか、共通の 1 本で足りるのか。
  エイリアスの条項は語形で数えられるので、最初の 1 本の候補になる
- 上の 4 機構をどう測り分けるか。入口 rule への 1 行と条項の例の追加は
  どちらも「載せる」系統で、同じ会話の返答に同時に効く
- grader が機械で判定できる条項はどれか。汎用語の実体は判定できるか
- 条項を載せない条件をどう作るか。常時ロードの rule を外した状態を再現する手段
- 測って差が出なかったときに何をするか。条項を消すのか、置き場を変えるのか、
  別の機構（hook・検査スクリプト）へ移すのか
- 7 件の条項の競合を、この段階で解くのか別に分けるのか
- 多段の参照（入口 rule → `core.md` → 分類別）の 2 段目が実行されたかを、どう測るか。
...
暫定。段階 3-2 より後の並びと同じく、段階 R が終わった時点で組み直す。
段階 4 とは独立で、時間軸で重ねられる。
