...
2026-09-03 時点。**段階 1・2・3-1 が完了し、段階 V を新設した。段階 3 は 3-1（実装の PR 群）と
3-2（保守の手引き）に割り、あいだに段階 R を入れた。3-1 の 9 本のうち 7 本
（PR 6・1・8・2・3・4・9）がマージ済みで、PR 5 は段階 R の R7 へ、PR 7 は 3-2 へ移した。
残る 2 本が両方とも外へ出たので 3-1 は閉じた。
norm-refit の中で次に着手するのは段階 R で、起点の R1（op-review を単機能 plugin として新設する）に着手済み。**
...
| V | 規範の遵守を測る仕組み | 条項がロードされた状態で守られるかが測れる |
...
条項がコンテキストに載った状態で、その条項が振る舞いを変えているかを測る仕組みを作る。
測定そのものは今回は流さない（ユーザー判断 2026-09-01）。この段階で決めるのは測り方で、
測るのは仕組みができてからになる。
