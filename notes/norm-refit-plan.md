...
2026-08-31 時点。**段階 1・2 が完了し、段階 3 は 9 本の PR のうち 7 本（PR 6・1・8・2・3・4・9）がマージ済み。
残っていた PR 5 は段階 R へ移し、PR 7 は段階 R の後に組み直す。次に着手するのは段階 R で、
起点は op-review を単機能 plugin として新設すること。**
...
| 3 | 実装（PR 群） | 到達点 1・2・3・5 が成立する。PR 7 を残して中断中 |
| R | レビュー機構の最適化 | レビュー 1 回の消費と反復回数が、決めた終了条件のもとで測れる |
| 4 | T4 の設計と実装 | 到達点 4 が成立し、保留 18 件が解ける |
| 5 | 完了処理 | 編集凍結が解け、norm-refit が終わる |

段階 R より後は暫定で、段階 R が終わった時点で組み直す（ユーザー判断 2026-09-01）。
並びの根拠は段階 R の「この段階の後の暫定の順序」にある。

段階 R は段階 3 と段階 4 のあいだに入る。番号ではなく `R`（review）を振るのは、
既存の段階 1〜5 の番号を他の文書（`.claude/rules/norm-refit-ops.md`、ccm-r003、台帳のエントリ）が
参照しており、振り直すと参照が全部ずれるため。
...
2026-08-31 に PR 5 を段階 R の R7 へ移した。段階 3 に残るのは PR 7 だけで、
それも段階 R の後に組み直す（ccm-f056 Q1）。
...
#### PR 7 japanese-text-writing の保守の手引き（段階 R の後に組み直す）

**2026-08-31 に着手を段階 R の後へ送った（ccm-f056 Q1）。中身も、そのときに組み直す。**
以下は送る前の骨子で、そのまま採るとは限らない。
...
1. PR 7（japanese-text-writing の保守の手引き）。段階 3 に残っている最後の 1 本。
   段階 R で規範の読ませ方が変わるので、その後に中身を組み直す
