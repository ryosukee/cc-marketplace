...
| 3 | 層の役割が実装されている | 入口 rule 約 20 行 / 詳細規範 6 ファイル（core + 分類別 5）/ 条件ロード / plugin の 4 層に条項が配られ、常時ロードは 9 ファイルのまま |
...
2026-08-23 時点。**段階 1・2 が完了し、PR #4 の着手前の判断も全部決着した。
段階 3 に着手済み: PR 6 が実装され PR #5 として draft で出ている。
残る実装は PR 1（core.md の条項単位再編）から。回答待ちの判断はゼロ。**
...
- 2026-08-25: PR 6 は PR #5 としてマージ済み。PR #6 はレビュー第 1 巡対応済みで未決 1 件の回答待ち。
  **PR 8（notes 運用 rule の改訂）を新設**（ccm-f044）。中身の残り 3 判断は ccm-f045 で回答待ち
...
進捗レポート `ccm-r003` は 2026-08-23 に改稿した（f038 Q9 の「再編後」をユーザー指示で前倒し）。次の改稿は PR #4 がマージできる状態になった時点。
...
- 条項ごとのロード方式（書く最中に効く条項 → `notes/**` の条件ロード rule を新設 /
  作る前・消す判断の条項 → 参照専用の notes-format.md に残す、が推奨案）
- 正典へ戻す範囲（3 項目 + 重複回避 + 消す手順 + 削除連動、が推奨案）
- 未コミット 2 件の作業ツリーの扱いと、primary-sources-first の条項の入れ方
